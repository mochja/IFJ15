...
INLINED result_t zval_eq(zval_t *dest, zval_t *a, zval_t *b) {
    /* @Tiboris  if a is equal b then res = 1 else res =0 */
    if (a == NULL || b == NULL) return ESYS; // TODO: Fix Error code
    else if (ZVAL_IS_DOUBLE(a) && ZVAL_IS_INT(b)) {
        int res = (ZVAL_GET_DOUBLE(a) == ZVAL_GET_INT(b)) ? 1 : 0;
        zval_set(dest, res);
    } else if (ZVAL_IS_DOUBLE(b) && ZVAL_IS_INT(a)) {
        int res = (ZVAL_GET_INT(a) == ZVAL_GET_DOUBLE(b)) ? 1 : 0;
        zval_set(dest, res);
    } else if (ZVAL_IS_DOUBLE(b) && ZVAL_IS_DOUBLE(a)) {
...
INLINED result_t zval_neq(zval_t *dest, zval_t *a, zval_t *b) {
    /* @Tiboris  if a not equal b then res = 1 else res =0 */
    if (a == NULL || b == NULL) return ESYS; // TODO: Fix Error code
    else if (ZVAL_IS_DOUBLE(a) && ZVAL_IS_INT(b)) {
        int res = (ZVAL_GET_DOUBLE(a) != ZVAL_GET_INT(b)) ? 1 : 0;
        zval_set(dest, res);
